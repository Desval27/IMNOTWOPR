...
namespace joshua {
enum class ConsoleNewline { raw, cr, lf, automatic };

inline ConsoleNewline parseConsoleNewline(const std::string& value) {
    if (value == "raw") return ConsoleNewline::raw;
    if (value == "cr") return ConsoleNewline::cr;
    if (value == "lf") return ConsoleNewline::lf;
    if (value == "auto") return ConsoleNewline::automatic;
...
inline const char* consoleNewlineName(ConsoleNewline mode) {
    switch (mode) {
    case ConsoleNewline::cr: return "cr";
    case ConsoleNewline::lf: return "lf";
    case ConsoleNewline::automatic: return "auto";
    default: return "raw";
    }
}
...
public:
    ...
    void setMode(ConsoleNewline mode) { mode_ = mode; previousCr_ = false; }
    void write(std::ostream& out, Byte value) {
        ...
        if (value == '\r' && expandCr) out << "\r\n";
        else if (value == '\n' && previousCr_ && expandCr) {} // CR already advanced the line.
        else if (value == '\n' && !previousCr_ && expandLf) out << "\r\n";
        else out.put(static_cast<char>(value));
        previousCr_ = value == '\r';
    }
private:
