...
void SerialSettings::validate() const {
    if (baud == 0 || baud > 4'000'000) throw std::runtime_error("serial baud must be 1..4000000");
    if (dataBits < 5 || dataBits > 8) throw std::runtime_error("serial data bits must be 5..8");
    if (stopHalfBits < 2 || stopHalfBits > 4) throw std::runtime_error("serial stop bits must be 1, 1.5 or 2");
    if (stopHalfBits == 3 && dataBits != 5) throw std::runtime_error("1.5 stop bits requires 5 data bits");
    parityName(parity);
}
...
Parity parseParity(std::string_view text) {
    ...
    if (value == "none" || value == "n") return Parity::none;
    if (value == "even" || value == "e") return Parity::even;
    if (value == "odd" || value == "o") return Parity::odd;
    if (value == "mark" || value == "m") return Parity::mark;
    if (value == "space" || value == "s") return Parity::space;
    throw std::runtime_error("serial parity must be none, even, odd, mark or space");
}
unsigned parseStopBits(std::string_view text) {
    if (text == "1") return 2;
    if (text == "1.5") return 3;
    if (text == "2") return 4;
