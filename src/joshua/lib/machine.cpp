...
namespace joshua
{
            ...
            for (const auto &region : regions)
            {
                ...
                if (!region.image.empty())
                {
                    ...
                    if (bytes.size() > region.size)
                        throw std::runtime_error("image exceeds mapping: " + region.image.string());
                    for (std::size_t i = 0; i < bytes.size(); ++i)
                        memory->patch(static_cast<Word>(i), bytes[i]);
                }
                bus.map({std::string(rom ? "ROM" : "RAM") + std::to_string(index++), region.base, region.size, memory});
            }
        };
        add(config.ram, false);
        add(config.rom, true);
        if (config.via)
        {
            via = std::make_shared<Via>();
            bus.map({"VIA", *config.via, 16, via});
        }
        if (config.acia)
        {
            acia = std::make_shared<Acia>(config.clockHz, config.serial);
            bus.map({"ACIA", *config.acia, 4, acia});
        }
        reset();
        if (config.pc)
            cpu.r.pc = *config.pc;
    ...
    {
        if (size > 65536u - address)
            throw std::runtime_error("save range exceeds $FFFF");
        std::vector<Byte> data(size);
        for (std::uint32_t i = 0; i < size; ++i)
            data[i] = bus.peek(static_cast<Word>(address + i));
        std::ofstream out(file, std::ios::binary | std::ios::trunc);
        if (!out)
            throw std::runtime_error("cannot create binary: " + file.string());
        out.write(reinterpret_cast<const char *>(data.data()), static_cast<std::streamsize>(data.size()));
        out.close();
        if (!out)
            throw std::runtime_error("failed writing binary: " + file.string());
    }
    std::string Machine::status() const
    {
        ...
        return "PC=" + hex(r.pc) + " A=" + hex(r.a, 2) + " X=" + hex(r.x, 2) + " Y=" + hex(r.y, 2) +
               ...
               (cpu.stopped() ? " STP" : cpu.waiting() ? " WAI"
                                                       : "") +
               "  " + disassemble(bus, r.pc);
    }
}
