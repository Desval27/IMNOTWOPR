...
void Memory::write(Word offset, Byte value) { if (!readOnly_) bytes_.at(offset) = value; }
...
void Bus::tick() {
    ++cycles_;
    for (const auto& m : mappings_) m.device->tick();
}
Byte Bus::read(Word address, bool sync) {
    auto m = locate(address);
    Byte value = m ? m->device->read(static_cast<Word>(address - m->base)) : 0xff;
    tick();
    if (trace) trace({cycles_, address, value, false, sync});
    return value;
}
void Bus::write(Word address, Byte value) {
    if (auto m = locate(address)) m->device->write(static_cast<Word>(address - m->base), value);
    tick();
    if (trace) trace({cycles_, address, value, true, false});
}
...
void Bus::idle() { tick(); }
void Bus::resetDevices() { for (const auto& m : mappings_) m.device->reset(); }
bool Bus::irq() const {
    for (const auto& m : mappings_) if (m.device->irq()) return true;
    return false;
}
}
