...
std::vector<std::string> tokenize(const std::string& text) {
    ...
    if (quoted) throw std::runtime_error("unterminated quoted argument");
...
MonitorAction Monitor::execute(const std::string& line) {
    ...
    if (args.empty()) return MonitorAction::prompt;
    ...
    if (command == "help" || command == "?") { arity(1, 1); out_ << help(); }
        ...
        if (args.size() == 3) {
            ...
            if (reg == "pc") cpu.r.pc = static_cast<Word>(value);
            else if (reg == "a") cpu.r.a = static_cast<Byte>(value);
            else if (reg == "x") cpu.r.x = static_cast<Byte>(value);
            else if (reg == "y") cpu.r.y = static_cast<Byte>(value);
            else if (reg == "sp") cpu.r.sp = static_cast<Byte>(value);
            else if (reg == "p") cpu.r.p = (static_cast<Byte>(value) & ~0x10) | 0x20;
            else throw std::runtime_error("unknown register");
        }
        ...
        for (std::uint32_t pos = 0; pos < size; pos += 16) {
            ...
            for (unsigned j = 0; j < 16 && pos + j < size; ++j) {
                Byte c = bus.peek(static_cast<Word>(address + pos + j)); out_ << (c >= 32 && c <= 126 ? static_cast<char>(c) : '.');
            }
            out_ << "|\n";
        }
        ...
        std::string source;
        for (std::size_t i = 2; i < args.size(); ++i) source += args[i] + " ";
        auto bytes = assemble(address, source); bus.patch(address, bytes);
        out_ << hex(address) << "  " << disassemble(bus, address) << "  next=$" << hex(static_cast<Word>(address + bytes.size())) << '\n';
    } else if (command == "load") {
        arity(3, 3); machine_.load(static_cast<Word>(n(2)), args[1]);
    } else if (command == "save") {
        arity(4, 4); machine_.save(static_cast<Word>(n(2)), n(3, 65536), args[1]);
    } else if (command == "map") {
        arity(1, 1);
        for (const auto& mapping : bus.mappings()) out_ << hex(mapping.base) << '-' << hex(mapping.base + mapping.size - 1) << " " << mapping.name << '\n';
    } else if (command == "break" || command == "b") {
        arity(1, 4);
        auto& points = machine_.breakpoints;
        if (args.size() == 1) {
            if (points.empty()) out_ << "No breakpoints.\n";
            for (const auto& [address, enabled] : points) out_ << hex(address) << (enabled ? " enabled\n" : " disabled\n");
        } else if (args.size() == 2) points[static_cast<Word>(n(1))] = true;
        else {
            Word address = static_cast<Word>(n(2));
            auto found = points.find(address);
            if (found == points.end()) throw std::runtime_error("no breakpoint at that address");
            if (args[1] == "delete" && args.size() == 3) points.erase(found);
            else if (args[1] == "enable" && args.size() == 3) found->second = true;
            else if (args[1] == "disable" && args.size() == 3) found->second = false;
            else if (args[1] == "move" && args.size() == 4) {
                Word destination = static_cast<Word>(n(3)); bool enabled = found->second;
                if (points.contains(destination)) throw std::runtime_error("breakpoint already exists at destination");
                points.erase(found); points[destination] = enabled;
            } else throw std::runtime_error("usage: break [ADDRESS | delete|enable|disable ADDRESS | move OLD NEW]");
        }
    } else if (command == "step" || command == "s" || command == "trace") {
        arity(1, 2); auto count = args.size() == 2 ? n(1, 1'000'000) : 1;
        if (cpu.stopped()) throw std::runtime_error("CPU is stopped; reset is required");
        auto oldTrace = bus.trace;
        if (command == "trace") bus.trace = [&](const BusCycle& c) {
            out_ << c.number << " " << (c.write ? 'W' : 'R') << " " << hex(c.address) << " " << hex(c.data, 2) << (c.sync ? " SYNC" : "") << '\n';
        };
        try { for (std::uint32_t i = 0; i < count && !cpu.stopped(); ++i) cpu.step(); }
        catch (...) { bus.trace = oldTrace; throw; }
        bus.trace = oldTrace; out_ << machine_.status() << '\n';
    } else if (command == "run" || command == "go" || command == "g" || command == "continue" || command == "c") {
        arity(1, 2);
        if (cpu.stopped()) throw std::runtime_error("CPU is stopped; reset is required");
        if (args.size() == 2) cpu.r.pc = static_cast<Word>(n(1));
        return MonitorAction::run;
    } else if (command == "reset") { arity(1, 1); machine_.reset(); }
    else if (command == "nmi") { arity(1, 1); cpu.nmi(); }
    else if (command == "io") {
        arity(3, 4);
        if (args[1] == "read" && args.size() == 3) out_ << hex(bus.read(static_cast<Word>(n(2))), 2) << '\n';
        ...
        for (std::size_t i = 1; i < args.size(); ++i) bytes.push_back(static_cast<Byte>(n(i, 255)));
        for (auto byte : bytes) machine_.acia->receive(byte);
