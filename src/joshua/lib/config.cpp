...
namespace joshua
{
    ...
    namespace
    {
            ...
            void apply(const std::string &key, std::string value, const std::filesystem::path &directory = {})
            {
                ...
                if (key == "ram" || key == "rom")
                {
                    ...
                    if (first == std::string::npos)
                        throw std::runtime_error(key + " needs base:size[:image]");
                    RegionConfig region{number(value.substr(0, first)),
                                        ...
                                        {}};
                    if (region.size == 0 || region.size > 65536 - region.base)
                        throw std::runtime_error("invalid " + key + " range");
                    if (second != std::string::npos)
                    {
                        std::string image = trim(value.substr(second + 1));
                        if (image.size() >= 2 && image.front() == '"' && image.back() == '"')
                ...
                else if (key == "via" || key == "acia")
                {
                    ...
                    if (value == "none")
                        ...
                        destination = static_cast<Word>(number(value));
                }
                else if (key == "clock-hz")
                {
                    config.clockHz = number(value, 100'000'000, 10);
                    if (!config.clockHz)
                        throw std::runtime_error("clock-hz must be positive");
                }
                else if (key == "serial-baud")
                    config.serial.baud = number(value, 4'000'000, 10);
                else if (key == "serial-data-bits")
                    config.serial.dataBits = number(value, 8, 10);
                else if (key == "serial-parity")
                    config.serial.parity = parseParity(value);
                else if (key == "serial-stop-bits")
                    config.serial.stopHalfBits = parseStopBits(value);
                else if (key == "console-newline")
                    config.consoleNewline = parseConsoleNewline(value);
                else if (key == "escape")
                {
                    ...
                    config.pc = static_cast<Word>(number(value));
                else
        ...
        void profile(Config &config, const std::filesystem::path &path)
        {
            ...
            if (in.bad())
                throw std::runtime_error("failed reading profile: " + path.string());
        }
    }
    Config parseArguments(int argc, char **argv)
    {
        ...
        for (int i = 1; i < argc; ++i)
        {
            ...
            else
            {
                ...
                {
                    if (profilePath)
                        throw std::runtime_error("only one --profile is allowed");
                    profilePath = value;
                }
                else if (arg == "--script")
                    result.script = value;
                else if (arg == "--cycles")
                {
                    ...
                    overrides.emplace_back(arg.substr(2), value);
            }
        }
    ...
    std::vector<Byte> readBinary(const std::filesystem::path &path)
    {
        ...
        if (size < 0 || size > 65536)
            throw std::runtime_error("binary must be at most 65536 bytes");
