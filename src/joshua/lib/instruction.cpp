...
std::uint32_t number(std::string_view text, std::uint32_t maximum, int base) {
    if (text.starts_with('$')) { base = 16; text.remove_prefix(1); }
    else if (text.size() > 2 && text[0] == '0') {
        switch (std::tolower(static_cast<unsigned char>(text[1]))) {
        case 'x': base = 16; text.remove_prefix(2); break;
        case 'd': base = 10; text.remove_prefix(2); break;
        case 'o': base = 8; text.remove_prefix(2); break;
        case 'b': base = 2; text.remove_prefix(2); break;
        default: break;
        }
    }
...
std::string disassemble(const Bus& bus, Word address) {
    ...
    if (i.nopCycles && opcode != 0xea) return ".BYTE $" + hex(opcode, 2);
    ...
    std::string operand;
    switch (i.mode) {
    case Mode::implied: break;
    case Mode::accumulator: operand = "A"; break;
    case Mode::immediate: operand = "#" + z; break;
    case Mode::zp: operand = z; break;
    case Mode::zpx: operand = z + ",X"; break;
    case Mode::zpy: operand = z + ",Y"; break;
    case Mode::absolute: operand = a; break;
    case Mode::absx: operand = a + ",X"; break;
    case Mode::absy: operand = a + ",Y"; break;
    case Mode::indx: operand = "(" + z + ",X)"; break;
    case Mode::indy: operand = "(" + z + "),Y"; break;
    case Mode::indirectZp: operand = "(" + z + ")"; break;
    case Mode::indirect: operand = "(" + a + ")"; break;
    case Mode::indirectX: operand = "(" + a + ",X)"; break;
    case Mode::relative:
        operand = "$" + hex(static_cast<Word>(address + 2 + static_cast<std::int8_t>(lo))); break;
    case Mode::bitBranch:
        operand = z + ",$" + hex(static_cast<Word>(address + 3 + static_cast<std::int8_t>(value >> 8))); break;
    }
    return std::string(i.name) + (operand.empty() ? "" : " " + operand);
}
std::vector<Byte> assemble(Word address, std::string text) {
    if (auto comment = text.find(';'); comment != std::string::npos) text.resize(comment);
    ...
    std::erase_if(operand, [](unsigned char c) { return std::isspace(c); });
    if (mnemonic == ".BYTE") return {static_cast<Byte>(number(operand, 255))};
    ...
    auto parse = [&](std::size_t start, std::size_t end) { return number(std::string_view(operand).substr(start, end - start)); };
    if (operand == "A") mode = Mode::accumulator;
    else if (branch) { mode = Mode::relative; target = number(operand); }
    else if (mnemonic.starts_with("BBR") || mnemonic.starts_with("BBS")) {
        auto comma = operand.find(',');
        if (comma == std::string::npos) throw std::runtime_error("bit branch needs zero-page address,target");
        mode = Mode::bitBranch; value = number(operand.substr(0, comma), 255); target = number(operand.substr(comma + 1));
    } else if (!operand.empty()) {
        if (operand[0] == '#') { mode = Mode::immediate; value = number(operand.substr(1), 255); }
        else if (operand[0] == '(') {
            if (operand.ends_with(",X)")) { mode = mnemonic == "JMP" ? Mode::indirectX : Mode::indx; value = parse(1, operand.size() - 3); }
            else if (operand.ends_with("),Y")) { mode = Mode::indy; value = parse(1, operand.size() - 3); }
            else if (operand.ends_with(')')) { mode = mnemonic == "JMP" ? Mode::indirect : Mode::indirectZp; value = parse(1, operand.size() - 1); }
            else throw std::runtime_error("invalid indirect operand");
        } else {
            ...
            if (mnemonic == "JMP" || mnemonic == "JSR") wide = true;
            mode = x ? (wide ? Mode::absx : Mode::zpx) : y ? (wide ? Mode::absy : Mode::zpy) : (wide ? Mode::absolute : Mode::zp);
        }
    ...
    auto lookup = [&](Mode wanted) -> int {
        for (int op = 0; op < 256; ++op) {
            ...
            if (i.name == mnemonic && i.mode == wanted && (!i.nopCycles || op == 0xea)) return op;
        }
        return -1;
    };
    int opcode = lookup(mode);
    if (opcode < 0) {
        if (mode == Mode::zp) mode = Mode::absolute;
        else if (mode == Mode::zpx) mode = Mode::absx;
        else if (mode == Mode::zpy) mode = Mode::absy;
        opcode = lookup(mode);
    }
    if (opcode < 0) throw std::runtime_error("unknown instruction or unsupported addressing mode");
    ...
    if (mode == Mode::relative || mode == Mode::bitBranch) {
        ...
        if (delta > 127 && delta < 0xff80) throw std::runtime_error("branch target is outside -128..127 bytes");
        if (mode == Mode::relative) value = delta & 255;
        else value |= (delta & 255) << 8;
    } else if (size == 2 && value > 255) throw std::runtime_error("operand must fit in one byte");
    std::vector<Byte> result{static_cast<Byte>(opcode)};
    if (size > 1) result.push_back(static_cast<Byte>(value));
    if (size > 2) result.push_back(static_cast<Byte>(value >> 8));
    return result;
}
}
