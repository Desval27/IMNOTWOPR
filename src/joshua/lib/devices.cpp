...
Byte Via::peek(Word offset) const {
    ...
    case 0: {
        ...
        if (registers_[11] & 0x80) value = (value & 0x7f) | (pb7_ ? 0x80 : 0);
        return value;
    }
    case 1: case 15: return (registers_[1] & registers_[3]) | (inputA_ & ~registers_[3]);
    case 4: return static_cast<Byte>(t1_);
    case 5: return static_cast<Byte>(t1_ >> 8);
    case 6: return static_cast<Byte>(latch1_);
    case 7: return static_cast<Byte>(latch1_ >> 8);
    case 8: return static_cast<Byte>(t2_);
    case 9: return static_cast<Byte>(t2_ >> 8);
    case 13: return ifr_ | (irq() ? 0x80 : 0);
    case 14: return ier_ | 0x80;
    default: return registers_[offset & 15];
    }
}
Byte Via::read(Word offset) {
    auto result = peek(offset);
    switch (offset & 15) {
    case 0: ifr_ &= ~0x18; break;
    case 1: ifr_ &= ~0x03; break;
    case 4: ifr_ &= ~0x40; break;
    case 8: ifr_ &= ~0x20; break;
    case 10: ifr_ &= ~0x04; break;
    default: break;
    }
    return result;
}
void Via::write(Word offset, Byte value) {
    switch (offset & 15) {
    case 0: registers_[0] = value; ifr_ &= ~0x18; break;
    case 1: registers_[1] = value; ifr_ &= ~0x03; break;
    case 15: registers_[1] = value; break;
    case 4: case 6: latch1_ = (latch1_ & 0xff00) | value; break;
    case 5:
        latch1_ = (latch1_ & 0xff) | (value << 8);
        t1_ = latch1_; armed1_ = true; delay1_ = true; reload1_ = false;
        pb7_ = false; ifr_ &= ~0x40; break;
    case 7: latch1_ = (latch1_ & 0xff) | (value << 8); ifr_ &= ~0x40; break;
    case 8: latch2Low_ = value; break;
    case 9:
        t2_ = static_cast<Word>((value << 8) | latch2Low_);
        armed2_ = true; delay2_ = true; ifr_ &= ~0x20; break;
    case 11:
        if (value & 0x1f) throw std::runtime_error("VIA shift register and input latching modes are not implemented yet");
        registers_[11] = value; break;
    case 12:
        if (value) throw std::runtime_error("VIA control-line modes are not implemented yet");
        registers_[12] = value; break;
    case 13: ifr_ &= ~(value & 0x7f); break;
    case 14:
        if (value & 0x80) ier_ |= value & 0x7f;
        else ier_ &= ~value;
        break;
    default: registers_[offset & 15] = value; break;
    }
}
void Via::count2() {
    if (t2_-- == 0 && armed2_) { ifr_ |= 0x20; armed2_ = false; }
}
void Via::setPortB(Byte value) {
    if ((registers_[11] & 0x20) && (inputB_ & 0x40) && !(value & 0x40)) count2();
    inputB_ = value;
}
void Via::tick() {
    if (delay1_) delay1_ = false;
    else if (reload1_) { t1_ = latch1_; reload1_ = false; }
    else if (t1_-- == 0) {
        reload1_ = true;
        if (armed1_) {
            ifr_ |= 0x40;
            if (registers_[11] & 0x40) pb7_ = !pb7_;
            else { armed1_ = false; pb7_ = true; }
        }
    }
    if (delay2_) delay2_ = false;
    else if (!(registers_[11] & 0x20)) count2();
}
void Via::reset() {
    registers_.fill(0); t1_ = t2_ = latch1_ = 0xffff; latch2Low_ = 0xff;
    ifr_ = ier_ = 0; armed1_ = armed2_ = delay1_ = delay2_ = reload1_ = false;
    pb7_ = true;
}
...
Byte Acia::peek(Word offset) const {
    ...
    case 1: return 0x10 | (full_ ? 8 : 0) | (overrun_ ? 4 : 0) | (interrupt_ ? 0x80 : 0);
    case 2: return command_;
    default: return control_;
    }
}
Byte Acia::read(Word offset) {
    auto value = peek(offset);
    if ((offset & 3) == 0) { full_ = overrun_ = false; }
    if ((offset & 3) == 1) interrupt_ = false;
    return value;
}
void Acia::write(Word offset, Byte value) {
    ...
    case 2:
        ...
        command_ = value; if (!(value & 1) || (value & 2)) interrupt_ = false;
        break;
...
void Acia::receive(Byte value) {
    if (input_.size() >= 65536) throw std::runtime_error("serial input queue is full");
