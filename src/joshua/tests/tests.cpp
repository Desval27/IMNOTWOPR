...
namespace
{
    ...
    void cpuTests()
    {
        ...
        f.cpu.reset();
        check(f.cpu.r.pc == 0x2000 && f.bus.cycles() == 7, "reset vector and cycles");
        f.cpu.r.pc = 0x200;
        f.code({0xa9, 0x42, 0x8d, 0x00, 0x30});
        check(f.cpu.step() == 2 && f.cpu.r.a == 0x42, "LDA immediate");
        check(f.cpu.step() == 4 && f.bus.peek(0x3000) == 0x42, "STA absolute");
        f.cpu.r.pc = 0x200;
        f.code({0xbd, 0xff, 0x30});
        f.cpu.r.x = 1;
        f.ram->patch(0x3100, 0x99);
        std::vector<BusCycle> trace;
        f.bus.trace = [&](auto c)
        { trace.push_back(c); };
        ...
        f.bus.trace = {};
        f.cpu.r.pc = 0x200;
        f.code({0x6c, 0xff, 0x30});
        f.ram->patch(0x30ff, 0x34);
        f.ram->patch(0x3100, 0x12);
        ...
        f.bus.patch(0xfffa, std::array<Byte, 2>{0x00, 0x40});
        f.cpu.nmi();
        ...
        f.cpu.reset();
        check(!f.cpu.stopped(), "reset releases STP");
    }
    void assemblerTests()
    {
        Fixture f;
        for (unsigned opcode = 0; opcode < 256; ++opcode)
        {
            ...
            for (std::size_t i = 1; i < bytes.size(); ++i)
                check(bytes[i] == f.bus.peek(static_cast<Word>(0x200 + i)), "operand round trip");
        }
        rejects([]
                { assemble(0x200, "BRA $0400"); });
        rejects([]
                { assemble(0x200, "LDA #$100"); });
    ...
    void busAndDeviceTests()
    {
        Config config;
        Machine m(config);
        m.bus.patch(0xc000, std::array<Byte, 1>{0x55});
        m.bus.write(0xc000, 0xaa);
        check(m.bus.peek(0xc000) == 0x55 && m.bus.peek(0x9000) == 0xff, "ROM protection and unmapped reads");
        rejects([&]
                { m.bus.map({"bad", 0, 1, std::make_shared<Memory>(1, false)}); });
        rejects([&]
                { m.bus.patch(0x7fff, std::array<Byte, 2>{1, 2}); });
        check(m.bus.peek(0x7fff) == 0, "failed patch must be atomic");
        m.bus.write(0x800e, 0xc0);
        m.bus.write(0x8004, 1);
        m.bus.write(0x8005, 0);
        check(!m.via->irq(), "timer not expired on write");
        m.bus.idle();
        check(!m.via->irq(), "timer decrement");
        m.bus.idle();
        check(m.via->irq(), "timer underflow IRQ");
        auto cycles = m.bus.cycles();
        m.bus.peek(0x8004);
        check(m.via->irq() && cycles == m.bus.cycles(), "peek leaves clock and IRQ unchanged");
        m.bus.read(0x8004);
        check(!m.via->irq(), "timer read acknowledges IRQ");
        for (unsigned i = 0; i < 12; ++i)
            m.bus.idle();
        check(!m.via->irq(), "one-shot timer does not retrigger on reload");
        m.bus.write(0x800b, 0xc0);
        m.bus.write(0x8005, 0);
        m.bus.idle();
        m.bus.idle();
        check(m.via->irq(), "free-run first timeout");
        m.bus.read(0x8004);
        m.bus.idle();
        m.bus.idle();
        check(m.via->irq(), "free-run retriggers at latch plus two cycles");
        m.bus.write(0x8013, 0x1f);
        m.bus.write(0x8012, 0x09);
        m.acia->receive('A');
        for (unsigned i = 0; i < 521; ++i)
            m.bus.idle();
        ...
        m.bus.write(0x8010, 'C');
        m.bus.write(0x8010, 'D');
    ...
    void monitorTests()
    {
        ...
        Monitor monitor(m, output);
        monitor.execute("asm 200 LDA #$42");
        monitor.execute("regs pc 200");
        monitor.execute("step");
        check(m.cpu.r.a == 0x42, "monitor assembly and step");
        rejects([&]
                { monitor.execute("write 300 01 100"); });
        check(m.bus.peek(0x300) == 0, "invalid write is atomic");
        monitor.execute("break 200");
        monitor.execute("break disable 200");
        monitor.execute("break move 200 300");
        check(!m.breakpoints.at(0x300), "breakpoint move preserves state");
        monitor.execute("break enable 300");
        check(m.breakpoints.at(0x300), "breakpoint enable");
        monitor.execute("break delete 300");
        check(m.breakpoints.empty(), "breakpoint deletion");
        check(monitor.execute("run 200") == MonitorAction::run, "run action");
        rejects([&]
                { monitor.execute("save \"unterminated 0 10"); });
        ...
        capture("dis 500 L 1");
        rejects([&]
                { monitor.execute("mem N 100"); });
        rejects([&]
                { monitor.execute("mem N L"); });
        rejects([&]
                { monitor.execute("mem N L 10000"); });
